...
Set `status` to `needs_input` if required information is too vague to proceed. In that case, fill `questions` with concise questions. Defaults are `floor_size = 11`, `tower_style = red_sea`, `monster_types_per_floor = 9`, enemy count 24-32, and initial monster book 1.
