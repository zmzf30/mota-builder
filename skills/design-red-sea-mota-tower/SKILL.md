---
name: design-red-sea-mota-tower
description: Understand and confirm the whole-tower brief and per-floor resource progression for a red-sea-style mota-js Magic Tower. Use as stage 0 of the code-orchestrated pipeline when tower_style is red_sea. Do not generate or review individual floor maps or edit project files.
---
...
Act as the stage-0 understanding agent for the red-sea branch of the classic Magic Tower pipeline.
...
- Set `tower_style` to exactly `red_sea`.
- Treat wall ratio 0.40-0.52 as a soft structural target, not the definition of density.
- Target 4-5 route families, 16-40 meaningful local opportunities, and 24-32 enemy tiles per floor unless the user explicitly overrides them.
- Require purposeful short wall groups, offset gaps, narrow route segments, frequent local branches and rejoins, and close macro-region density.
- Keep resource locations spatially dispersed and access costs staged; do not flatten the floor into identical regional recipes.
- Reserve enough usable cells for every exact resource quota, the enemy minimum, stairs, and at least 18 ordinary empty-ground cells.
...
- Red-sea floors place 24-32 enemy tiles by default. Explicit user settings remain authoritative.
...
When the user keeps the red-sea resource defaults, allocate red gems, blue gems, and red potions in a non-decreasing 6-8 per-floor rhythm centered on 7; allocate 3 blue potions, 2 yellow keys, 1 blue key, 1 pickaxe, 1 bomb, and 1 centerFly per floor. Keep every whole-tower total exact. For non-default totals, distribute the exact integers progressively instead of putting every remainder on the opening floors.
...
```json
{
  ...
  "monster_policy": {
    ...
    "enemy_count_min_per_floor": 24,
    "enemy_count_max_per_floor": 32,
  ...
  "floor_progression_plan": [
    {
      ...
      "resource_budget": {
        ...
        "pickaxes": 1, "bombs": 1, "centerFly": 1, "jumpShoes": 0,
        "redGems": 6, "blueGems": 6, "greenGems": 0,
        "redPotions": 6, "bluePotions": 3, "yellowPotions": 0, "greenPotions": 0
      },
...
Set `status` to `needs_input` if required information is too vague to proceed. In that case, fill `questions` with concise questions. Defaults are `floor_size = 11`, `tower_style = red_sea`, `monster_types_per_floor = 12`, a 10-12 type candidate catalog, enemy count 24-32, and initial monster book 1.
