...
- Real few-shot reference input is enabled by default from the repository-bundled `references/few-shot/corpus.json`. It injects selected real maps, route graphs, resource reachability, and used enemy data without depending on a user-home directory. Per floor and stage, generator and reviewer share one calibration anchor but receive different exclusive examples; reviewer-exclusive examples are holdouts and reviewer prompts also receive contrastive rejection cases. The split is written once to `few_shot_selection.json` and reused across retries. Use `--few-shot-root` to rebuild from folders containing `project/`, `--few-shot-corpus` for another extracted corpus, `--few-shot-count` to tune examples per consumer, or `--no-few-shot` only when explicitly requested.
- Use `--tower-style traditional|red_sea`; default to `traditional`. Few-shot selection is strict by style and never falls back across the two groups. Style controls layout and placement grammar, not numeric combat difficulty.
...
- The paths to `few_shot_corpus.json` and `few_shot_selection.json` when real reference input is enabled.
- The enemy-table calibration artifacts `hero_projection.json`, `enemy_floor_plan.json`, `enemy_floor_analysis.json`, and `enemy_design.review.json`.
