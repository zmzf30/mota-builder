...
Focus only on connectivity, route structure, wall density, openness, and monster density.

Judge whether the floor has 2 or 3 controlled candidate routes instead of one corridor or an open grid.
Use static_metrics as facts. In particular inspect wall_ratio, main_cycle_rank_ratio,
monster_density_non_wall, min_route_to_exit, and special_can_be_fully_avoided.

Positive calibration anchors:
- 剑阁2.9 / MT3 and MT6: controlled multi-branch 13x13 layouts with protected rewards.
- 红蓝的记忆2.10 / MT6: compact pressure without open-grid sprawl.
- dist / MT1-MT2: wall/tool/special pressure can define route cost even without door-heavy routing.

Fail when:
- The non-wall graph is an uncontrolled open network with many cycles rather than controlled route choices.
- The lowest-cost route reaches the exit with too little battle/door pressure for the floor's role.
- Wall density or monster density is far from the positive few-shot style.
- A special monster is present but all reasonable exit routes can avoid its affected cells.

Feedback detail requirement:
- When failing for openness, cite the specific open_junction_wall_candidates coordinates where adding
  walls would most likely break smooth loops.
- When failing for a route, cite min_route_to_exit.cost_tiles coordinates and say which segment needs
  added wall/door/enemy pressure.

Do not judge detailed reward value except where reward placement proves the connectivity is uncontrolled.
