...
│   ├── economy-mota-floor/     # 钥匙、道具、资源与压力意图
│   ├── encounter-mota-floor/   # 门、怪物与特殊压力联合设计
...
- 传统塔默认墙比例 `0.35-0.45`、2-4 条主要路线、6-20 个有效决策点、18-28 个怪物；红海塔使用独立 generator/reviewer skills，默认墙比例软目标 `0.40-0.52`、4-5 条路线族、16-40 个有效决策点和 24-32 个怪物，并校验分区密度与墙/路线碎片化。两种风格默认每层提供 10-12 个角色候选，默认上限为 12 种怪物。传统塔仍以 2-6 个特殊压力位置、平均每层 0.5-2 个工具、70%-90% 重要资源保护率和 0%-10% 钥匙/资源安全余量为目标。用户或已确认 brief 的显式设置优先。
- WebUI 红海塔默认每层按 6-8 个红宝石、蓝宝石和红血瓶的非递减节奏分配（整塔总量以 7 个/层为中心），并默认每层 3 个蓝血瓶、2 把黄钥匙、1 把蓝钥匙、1 个破墙镐、1 个炸弹和 1 个中心对称飞行器；传统塔资源默认值不变。
- WebUI 传统塔默认整塔资源为红宝石、蓝宝石、红血瓶各 `6 × 层数`，蓝血瓶 `1 × 层数`；红海塔使用上一条所述的独立高密度资源默认值。
- 不论选择哪种风格，开始生成前都会强制把初始怪物手册 `book` 设置为 `1`。
- 生成阶段会分为怪物数据设计与专用审查、拓扑、经济、怪物特殊能力、审查和修复。
- 最终 encounter 后会运行“BFS 区域压缩 + 有界 Pareto Dijkstra-DP”路线平衡检查：安全连续空地合并为区域，宝石、门、怪物和领域/阻击覆盖格保留为微节点；仅审查含红/蓝/绿宝石的资源区，动态应用途中宝石攻防/护盾成长，计算战斗、领域、阻击伤害，并按黄钥匙 `1.5`、蓝钥匙 `2.5` 个红血瓶折算路线成本。同资源的前三个非支配路线族需满足 `最优成本 / 第三优成本 >= 0.8`。搜索默认最多扩展 `40,000` 个标签或运行 `3` 秒；失败时由专用 repair agent 仅调整门和怪物，最终仍不通过则确定性选择平衡度最高的合法候选。
...
- 怪物表 reviewer 按 8 类角色的 ATK/DEF/HP 合约、每层 10-12 个角色槽、跨层复用门槛和强怪生态位检查；不要求跨层复用怪物重新满足当前层原始比例，也不要求候选怪全部立即可击杀。除魔攻外，怪物 ATK 不能低于本层目标勇士 DEF；不允许连续跨 3 层、上层怪物占据本层强怪槽，跨层 ATK 必须达到当前层 completion.DEF 的 1.3 倍。`numeric_score` 仅用于排序辅助，不作为核心难度指标。红海塔首次进入某层候选池时仍必须满足怪物 `ATK+DEF >=` 该层目标勇士 `ATK+DEF`。结果写入 `hero_projection.json`、`enemy_floor_plan.json`、`enemy_floor_analysis.json` 和 `enemy_design.review.json`。
...
- 完整楼层被 reviewer 拒绝后会保留为候选；达到最大尝试次数时，最后一轮生成完成后由 reviewer 从全部完整候选中选出一个，不再直接保存最新结果。
