...
- 传统塔默认墙比例 `0.35-0.45`、2-4 条主要路线、6-20 个有效决策点、18-28 个怪物；红海塔使用独立 generator/reviewer skills，默认墙比例软目标 `0.40-0.52`、4-5 条路线族、16-40 个有效决策点和 24-32 个怪物，并校验分区密度与墙/路线碎片化。两种风格默认每层最多 9 种怪物。传统塔仍以 2-6 个特殊压力位置、平均每层 0.5-2 个工具、70%-90% 重要资源保护率和 0%-10% 钥匙/资源安全余量为目标。用户或已确认 brief 的显式设置优先。
...
- 怪物表 reviewer 只检查整体强度层级、角色多样性和跨层原始强度进步，不要求候选怪能被本层投影勇士立即击杀，也不输出 warning；红海塔怪物首次进入某层候选池时必须满足怪物 `ATK+DEF >=` 该层目标勇士 `ATK+DEF`，后续楼层复用不重复检查。结果写入 `hero_projection.json`、`enemy_floor_plan.json`、`enemy_floor_analysis.json` 和 `enemy_design.review.json`。
